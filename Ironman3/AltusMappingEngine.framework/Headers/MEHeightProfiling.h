...
/** Returns an array of MEHeightSample objects along a given route. This function may be run on a background thread.
 @param terrainPackages An array of filenames (NSString objects), one for each terrain map package to sample from.
 ...
 */
+ (NSArray*) getTerrainProfile:(NSArray*) terrainPackages
 ...
 */
+ (CGPoint) getMinMaxTerrainHeightsInBoundingBox:(NSArray*) terrainPackages
 ...
 */
+ (CGPoint) getMinMaxTerrainHeightsAroundLocation:(NSArray*) terrainPackages
...
@interface MEProfile : NSObject

+ (void) sample:(NSString*)path;
